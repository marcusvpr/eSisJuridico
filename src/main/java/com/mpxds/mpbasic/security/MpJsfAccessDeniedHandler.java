...
public class MpJsfAccessDeniedHandler implements AccessDeniedHandler {
    ...
    public void trataMpSistemaLog() {
		...
		mpSistemaLog.setCodigo("MpJsfAccessDeniedHandler");
		mpSistemaLog.setMensagem("Você não possui permissão para acessar esta página");
		...
		try {
	      ...
	      // execute a query
	      String sql = 
	    		  "INSERT INTO mp_sistema_log (data_criacao, codigo, mensagem, tenant_id) VALUES (" +
	    				  	"'" + df.format(mpSistemaLog.getDataCriacao()) + "', " +
	    				  	"'" + mpSistemaLog.getCodigo() + "', " +
	    				  	"'" + mpSistemaLog.getMensagem() + "', '0')";
	      //
