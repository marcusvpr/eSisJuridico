...
@Named
@ViewScoped
public class MpPesquisaImagemBDsBean implements Serializable {
	...
	public MpPesquisaImagemBDsBean() {
		...
		model = new LazyDataModel<MpArquivoBD>() {
			...
			public List<MpArquivoBD> load(int first, int pageSize, String sortField,
				...
				mpFiltro.getMpFilterOrdenacao().setAscendente(SortOrder.ASCENDING.equals(
																					sortOrder));
				setRowCount(mpImagemBDs.quantidadeFiltrados(mpFiltro));
				
				return mpImagemBDs.filtrados(mpFiltro);
			}
