...
import com.mpxds.mpbasic.repository.filter.MpCompraFilter;
import com.mpxds.mpbasic.security.MpSeguranca;
import com.mpxds.mpbasic.util.jpa.MpTransactional;

public class MpCompras implements Serializable {
	...
	public Map<Date, BigDecimal> valoresTotaisPorData(Integer numeroDeDias,	MpUsuario criadoPor) {
		...
		Criteria criteria = session.createCriteria(MpCompra.class);
		
		// select date(data_criacao) as data, sum(valor_total) as valor 
		// from mpCompra where data_criacao >= :dataInicial and vendedor_id = :criadoPor 
...
//			.add(Restrictions.ge("dataCriacao", dataInicial.getTime()));
		...
		//
//		System.out.println("MpCompras.valoresTotaisPorData - ( resultado = " + resultado.size());
		//
		return resultado;
	}
	...
	private Criteria criarCriteriaParaFiltro(MpCompraFilter filtro) {
		//
		Session session = this.manager.unwrap(Session.class);
		
		Criteria criteria = session.createCriteria(MpCompra.class)
				.createAlias("mpContato", "c")
		...
		if (StringUtils.isNotBlank(filtro.getNomeContato())) {
			// acessamos o nome do cliente associado ao mpCompra pelo alias "c", 
			// criado anteriormente
			criteria.add(Restrictions.ilike("c.nome", filtro.getNomeContato(), 
																		MatchMode.ANYWHERE));
		}
		//
		return criteria;
	...
	@SuppressWarnings("unchecked")
	public List<MpCompra> filtrados(MpCompraFilter filtro) {
		//
	...
	public int quantidadeFiltrados(MpCompraFilter filtro) {
		//
		Criteria criteria = criarCriteriaParaFiltro(filtro);
	...
	public MpCompra guardar(MpCompra mpCompra) {
		//
		try {
			return this.manager.merge(mpCompra);
			//
	...
	public MpCompra porId(Long id) {
		//
		return this.manager.find(MpCompra.class, id);
	}
...
//															mpSistemaConfig.getValor());
	...
	@MpTransactional
	public void remover(MpCompra mpCompra) throws MpNegocioException {
		//
		try {
			mpCompra = porId(mpCompra.getId());
			
			manager.remove(mpCompra);
			manager.flush();
			//
		} catch (PersistenceException e) {
			throw new MpNegocioException("Compra... não pode ser excluído.");
		}
	}

	public MpCompra porNavegacao(String acao, Date dataCriacao) {
		//
//		System.out.println("MpCompras.MpCompra ( " + acao + " / " + descricao);
		//		
		...
		try {
			...
			if (acao.equals("mpEnd") || acao.equals("mpPrev")) {
				hql = "from MpCompra where dataCriacao < :dataCriacao";
			...
			if (hql.isEmpty())
				...
				return manager.createQuery(hql,	MpCompra.class)
