...
import com.mpxds.mpbasic.repository.filter.MpMensagemMovimentoFilter;
...
public class MpMensagemMovimentos implements Serializable {
	...
	private Criteria criarCriteriaParaFiltro(MpMensagemMovimentoFilter filtro) {
		...
		if (StringUtils.isNotBlank(filtro.getUsuario()))
			criteria.add(Restrictions.ilike("mpUsuario.nome", filtro.getUsuario(), 
																			MatchMode.ANYWHERE));
		if (StringUtils.isNotBlank(filtro.getContato()))
			criteria.add(Restrictions.ilike("contatoNome", filtro.getContato(), 
																			MatchMode.ANYWHERE));
	...
	public MpMensagemMovimento porEmailData(String email, Date dataMensagem) {
		//
		try {
			//
			return manager.createQuery(
					"from MpMensagemMovimento where upper(email) = :email AND " +
	...
	public List<MpMensagemMovimento> byDataList() {
		return this.manager.createQuery("from MpMensagemMovimento ORDER BY dataMovimento",
													MpMensagemMovimento.class).getResultList();
	}
