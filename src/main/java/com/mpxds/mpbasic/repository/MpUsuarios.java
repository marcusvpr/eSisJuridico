...
//import com.mpxds.mpbasic.model.MpGrupoObjeto;
...
public class MpUsuarios implements Serializable {
	...
	public MpUsuario porNome(String nome) {
		try {
			return manager.createQuery(
				"from MpUsuario where upper(nome) = :nome", MpUsuario.class)
	...
	public List<MpObjeto> porMpGrupoMenu(MpGrupoMenu mpGrupoMenu, List<MpGrupo> mpGrupoList) {
		//
//		System.out.println("MpUsuarios.porMpGrupoMenu - 000 ( " + mpGrupoMenu.getDescricao() +
//																" / " + mpGrupoList.size());
		//
//		Set<MpObjeto> mpObjetoSet = new HashSet<MpObjeto>();
        SortedSet<MpObjeto> mpObjetoSortedSet = new TreeSet<>(Comparator.comparing(MpObjeto::
        																	getMpGrupamentoNome));

//    	Iterator<MpGrupo> itr = mpGrupoList.iterator(); 
    	//
//    	while(itr.hasNext()) {
    	for (MpGrupo mpGrupo : mpGrupoList) {
	    	//
//    		MpGrupo mpGrupo = (MpGrupo) itr.next();
		
//			String myQuery = "FROM MpGrupoObjeto mgo" +
//							" WHERE mgo.mpStatusObjeto = 'ATIVO' AND" +
//							" mgo.mpGrupo.nome = :mpGrupo";
//							 " order by mpGrupoMenu";
			//
//			mpGrupoObjetoSet.addAll(this.manager.createQuery(myQuery, MpGrupoObjeto.class).
//										setParameter("mpGrupo", mpGrupo.getNome()).getResultList());

			mpObjetoSortedSet.addAll(mpGrupo.getMpObjetos());
    		
//    		System.out.println("MpUsuarios.porMpGrupoMenu (mpGrupo = " + mpGrupo.getDescricao() +
//    																" / " + mpObjetoSet.size());
    	}
		//
		List<MpObjeto> mpObjetoList = new ArrayList<MpObjeto>();

//    	Iterator<MpObjeto> itrObj = mpObjetoSet.iterator(); 
    	//
